...
Clonare il repository  
   ```bash
   git clone https://github.com/tuo-username/tasknova.git
   cd tasknova ```bash
...
* **Popolare il database (opzionale):**
    Se il progetto include dei dati di prova o iniziali, puoi inserirli eseguendo:
    ```bash
    php artisan db:seed
    ```
...
* **Avviare il server Laravel:**
    Apri un nuovo terminale (lasciando `npm run dev` in esecuzione in background, se necessario) ed esegui:
    ```bash
    php artisan serve
    ```

* **Visualizzare l'applicazione:**
    Apri il tuo browser di preferenza e naviga su: [http://localhost:8000](http://localhost:8000)